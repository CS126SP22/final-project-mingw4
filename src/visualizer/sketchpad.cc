#include <visualizer/sketchpad.h>

namespace cellautomata {

namespace visualizer {

using glm::vec2;

Sketchpad::Sketchpad(const vec2& top_left_corner, size_t num_pixels_per_side,
                     double sketchpad_size, double brush_radius)
    : top_left_corner_(top_left_corner),
      num_pixels_per_side_(num_pixels_per_side),
      pixel_side_length_(sketchpad_size / num_pixels_per_side),
      brush_radius_(brush_radius) {
    
    for (size_t i = 0; i < num_pixels_per_side_; i++) {
      std::vector<unsigned char> row;
      std::vector<unsigned char> row_temp;
      for (size_t j = 0; j < num_pixels_per_side_; j++) {
        row.push_back(0);
        row_temp.push_back(0);
      }
      sketched_image_.push_back(row);
      sketched_image_temp_.push_back(row_temp);
    }
    ;
}

void Sketchpad::Draw() const {
  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++col) {
      if (sketched_image_[row][col] == 1) {
        ci::gl::color(ci::Color::gray(0.3f));
      } else {
        ci::gl::color(ci::Color("white"));
      }

      vec2 pixel_top_left = top_left_corner_ + vec2(col * pixel_side_length_,
                                                    row * pixel_side_length_);

      vec2 pixel_bottom_right =
          pixel_top_left + vec2(pixel_side_length_, pixel_side_length_);
      ci::Rectf pixel_bounding_box(pixel_top_left, pixel_bottom_right);

      ci::gl::drawSolidRect(pixel_bounding_box);

      ci::gl::color(ci::Color("black"));
      ci::gl::drawStrokedRect(pixel_bounding_box);
    }
  }
}

void Sketchpad::HandleBrush(const vec2& brush_screen_coords) {

  vec2 brush_sketchpad_coords =
      (brush_screen_coords - top_left_corner_) / (float)pixel_side_length_;

  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++col) {
      vec2 pixel_center = {col + 0.5, row + 0.5};

      if (glm::distance(brush_sketchpad_coords, pixel_center) <=
          brush_radius_) {
        sketched_image_[row][col] = 1;
      }
    }
  }
}

void Sketchpad::Clear() {

  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++col) {
      sketched_image_[row][col] = 0;
    }
  }
}

void Sketchpad::Step() {

  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++ col) {

      int count_nighbors_alive = 0;
      for (int i = -1; i < 2; i ++) {
        for (int j = -1; j < 2; j ++) {
          if (i == 0 && j == 0)
            continue;
          size_t _i = (i + row) % num_pixels_per_side_;
          size_t _j = (j + col) % num_pixels_per_side_;
          if (sketched_image_[_i][_j]) {
            count_nighbors_alive += 1;
          }
        }
      }

      if (sketched_image_[row][col] == 1) {
        if (count_nighbors_alive == 2 || count_nighbors_alive == 3) {
          sketched_image_temp_[row][col] = 1;
        } else {
          sketched_image_temp_[row][col] = 0;
        }
      } else {
        if (count_nighbors_alive == 3) {
          sketched_image_temp_[row][col] = 1;
        } else {
          sketched_image_temp_[row][col] = 0;
        }
      }
    }
  }

  for (size_t row = 0; row < num_pixels_per_side_; ++row) {
    for (size_t col = 0; col < num_pixels_per_side_; ++ col) {
      sketched_image_[row][col] = sketched_image_temp_[row][col];
    }
  }

}

}  // namespace visualizer

}  // namespace cellautomata
